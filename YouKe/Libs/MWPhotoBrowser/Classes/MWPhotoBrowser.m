...
- (void)viewDidLoad {
    ...
    if (self.displayDeleteButton) {
        _deleteButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"UIBarButtonItemDelete.png"] style:UIBarButtonItemStylePlain target:self action:@selector(deleteButtonPressed:)];
    }
    if (![self.displaySendButton isEqualToString:@""] && self.displaySendButton != nil)
    {
        UIButton *tempButton = [UIButton buttonWithType:UIButtonTypeCustom];
        tempButton.frame = CGRectMake(0, 0, 66.5, 31);
        [tempButton setBackgroundImage:[UIImage imageNamed:@"iOS-searUser_sBtnN.png"] forState:UIControlStateNormal];
        [tempButton setBackgroundImage:[UIImage imageNamed:@"iOS-searUser_sBtnD.png"] forState:UIControlStateHighlighted];
        [tempButton setTitle:self.displaySendButton forState:UIControlStateNormal];
        [tempButton setTitle:self.displaySendButton forState:UIControlStateHighlighted];
        [tempButton addTarget:self action:@selector(sendButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        _sendButton = [[UIBarButtonItem alloc] initWithCustomView:tempButton];
        _sendButton.title = self.displaySendButton;
    }
...
- (void)updateNavigation {
    ...
    _deleteButton.enabled = [[self photoAtIndex:_currentPageIndex] underlyingImage] != nil;
	_sendButton.enabled = [[self photoAtIndex:_currentPageIndex] underlyingImage] != nil;
}
...
- (void)sendButtonPressed:(id)sender
{
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage])
    {
        if ([self.delegate respondsToSelector:@selector(photoBrowser:sendButtonPressedForPhotoAtIndex:)])
        {
            [self.delegate photoBrowser:self sendButtonPressedForPhotoAtIndex:_currentPageIndex];
        }
    }
}
